...
const firebaseConfig = {
  ...
  databaseURL:
    "https://telegram-bot-database-a8986-default-rtdb.europe-west1.firebasedatabase.app",
...
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(msg.chat.id, `Welcome! \nAlmost there! 🚀 To see who’s near you, please share your location. 
      \nEnsure you add a username so your friends can reach you`,
    {
      reply_markup: {
        keyboard: [
          [
...
bot.on("location", async (msg) => {
  ...
  bot.sendMessage(
    msg.chat.id,
    `Hello ${username} ${firstName} Thanks for sharing 
        ...
        \nIf you get an error or dont see any contacts near you, go to settings/permission and allow Telegram to access your location. Then press /start again`
  );
  ...
  //      remove(ref(database, `nearbyuser/${users}`));
  //     return;
  //   }
    ...
    for (var i = 0; i < dataList.length; i++) {
      ...
      const a =
        ...
        Math.cos((latitude * Math.PI) / 180) *
          Math.cos((dataList[i].latitude * Math.PI) / 180) *
          Math.sin(dLon / 2) *
          Math.sin(dLon / 2);
      ...
      if (distance <= 8) {
        bot.sendMessage(
          msg.chat.id,
          `@${dataList[i].username} ${dataList[i].firstName} - is ${distance}km away`
        );
...
bot.onText("No, thanks", (msg) => {
  bot.sendMessage(
    msg.chat.id,
    "No problem! If you change your mind, just type /start to share your location."
  );
});
...
bot.onText(/\/stop/, (msg) => {
  bot.sendMessage(
    msg.chat.id,
    "You have stopped sharing your location. If you want to share it again, just type /start."
  );
});
